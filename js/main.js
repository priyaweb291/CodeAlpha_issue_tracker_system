function submitIssue(e) {
  ...
  const fileUpload = getInputValue('issueFileUpload');
  const id = Math.floor(Math.random() * 100000000) + '';
  const status = 'Open';

  if ((description.length == 0) || (assignedTo.length == 0 || (fileUpload.length == 0))) {
    ...
    const issue = { id, description, severity, assignedTo, fileUpload, status };
    let issues = [];
...
const fetchIssues = () => {
  ...
  for (var i = 0; i < issues.length; i++) {
    const { id, description, severity, assignedTo, fileUpload, status } = issues[i];
    ...
    issuesList.innerHTML += `<div class="well">
                              ...
                              <p><span class="glyphicon glyphicon-user"></span> ${fileUpload}</p>
